...
DRs1 = []
for i in range(1,11):
    gen_data = {"n":7000, "p":1000, "k":i*10, "alpha":1, "beta_star":0.1, "dat_type":1}
    result_1000_beta1 = OLS_fea_numexp(gen_data, 100, 0)
# DR_exp2 = np.sum(result_1000_beta1[:,1] == gen_data["k"])
    PCD_exp2 = np.mean(result_1000_beta1[:,1] / gen_data["k"])
    DRs1.append(PCD_exp2)
plt.rcParams['figure.dpi'] = 800
plt.plot(np.arange(1,11)*10, DRs1)
plt.title('True Recovery, n = 7000, p = 1000, beta = 0.1')
plt.ylabel('Detection Rate')
plt.xlabel('True Sparsity Level k*')
